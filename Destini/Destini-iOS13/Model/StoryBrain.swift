...
struct StoryBrain{
    ...
    let examples = [    Story( title: "Your car has blown a tire on a winding road in the middle of nowhere with no cell phone reception. You decide to hitchhike. A rusty pickup truck rumbles to a stop next to you. A man with a wide brimmed hat with soulless eyes opens the passenger door for you and asks: 'Need a ride, boy?'.",
   ...
   mutating func nextStory(_ userchoice: String){
        if (userchoice == examples[count].choice1) {
            count  = examples[count].choice1Destination
        } else {
            count  = examples[count].choice2Destination
        }
    }
    
    func getTitle()->String{
        return examples[count].title
    }
    func getChoice1()->String{
        return examples[count].choice1
    }
    func getChoice2()->String{
        return examples[count].choice2
    }
}
